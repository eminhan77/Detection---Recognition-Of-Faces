...
dataset_path = 'custom_train_img'  # LFW veri seti dizini
    ...
    for img_name in os.listdir(person_path):
        ...
        if faces_detected is not None:
            for face in faces_detected:
                ...
                custom_faces.append(embedding)
                custom_labels.append(label)  # Etiket olarak kişiyi ekle

# Modeli kaydetmek için pickle veya numpy formatını kullanabiliriz
np.save('custom_faces.npy', custom_faces)  # Yüz özelliklerini kaydediyoruz
np.save('custom_labels.npy', custom_labels)  # Etiketleri kaydediyoruz
print("Yeni Eğitim tamamlandı ve model kaydedildi!")
