...
# Yüz algılama için MTCNN(multi-task convolutional neural network) modelini yükleyelim
...
# the faces and labels are assumed in an array
faces = np.load('faces.npy', allow_pickle=True) # .npy files are binary files which stores numpy arrays
labels = np.load('labels.npy', allow_pickle=True)

# Veri seti yolu ve kişi isimlerini al
dataset_path = 'lfw-deepfunneled'  # LFW veri seti dizini, approximately 4749 pictures in that dataset
person_names = os.listdir(dataset_path)  # Klasörlerdeki kişi isimlerini al

# Test fotoğrafını yükleyelim
test_image_path = "testimg10.jpg"  # Test fotoğrafının yolu
test_img = cv2.imread(test_image_path) # img is read
test_img_rgb = cv2.cvtColor(test_img, cv2.COLOR_BGR2RGB) # conversion into rgb
...
if faces_in_test is not None:   # if there are some faces
